...
def main():
    my_parser = argparse.ArgumentParser(prog='proxy_scraper',
                                        description='The proxy scraper script using ProxyBroker',
                                        allow_abbrev=False)
    my_parser.add_argument('-p', '--path', type=str,
                           help="Path of file the collected proxies will write in", default='proxies.txt')
    my_parser.add_argument('-l', '--limit', type=int,
                           help="Limit parameter for proxybroker", default=100)
    my_parser.add_argument('-s', '--settings', type=str,
