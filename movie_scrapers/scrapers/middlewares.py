...
class CustomRotatingProxiesMiddleware(RotatingProxyMiddleware):
    ...
    def __init__(self, proxy_list, logstats_interval, stop_if_no_proxies,
        ...
        super(CustomRotatingProxiesMiddleware, self).__init__(proxy_list,
                                                              logstats_interval, stop_if_no_proxies,
                                                              max_proxies_to_try, backoff_base, backoff_cap, crawler)
        # change default proxy class with custom one
        self.proxies = CustomProxies(self.cleanup_proxy_list(proxy_list), backoff=self.proxies.backoff)
...
class CustomProxies(Proxies):
    ...
    def __init__(self, proxy_list, backoff=None):
        ...
        if collection_interval > 0:
            ...
            self.task = RepeatedTimer(self.update_proxies, collection_interval * 60,
                                      event_loop=worker_loop, now=False, read_from_file=False)
            self.task.start()
    ...
    def update_proxies(self, read_from_file=True, read_from_broker=True):
        ...
        new_proxies = CustomProxies.get_proxies(read_from_file=read_from_file, read_from_broker=read_from_broker)
    ...
    def get_proxies_from_file():
        ...
        if proxy_path is not None and os.path.isfile(proxy_path):
            proxy_list = CustomProxies.check_proxies(open(proxy_path, "r", encoding="utf-8"))
            logger.info(f'[Thread: {"Main" if is_main_thread else "Not main"}]'
    ...
    def get_proxies_from_external(cls):
        ...
        def scrape_proxies():
            ...
            scrape_settings = json.dumps({'dnsbl': s.get('PROXY_DNSBL'),
        ...
        with cls.gather_lock:
            ...
            try:
                ...
                logger.error(f'[Thread: {"Main" if is_main_thread else "Not main"}]'
                             f'{e}. '
    ...
    def get_proxies_programmatically(cls):
        ...
        with cls.gather_lock:
            ...
            try:
                tasks = asyncio.gather(broker.find(types=s.get('PROXY_TYPES'),
                                                   countries=s.get('PROXY_COUNTRIES'),
                                                   strict=True,
    ...
    def check_proxies(cls, proxy_list):
        ...
        with cls.check_lock:
            ...
            try:
                tasks = asyncio.gather(broker.find(data=proxy_list,
                                                   types=s.get('PROXY_TYPES'),
                                                   countries=s.get('PROXY_COUNTRIES'),
                                                   strict=True,
                                                   dnsbl=s.get('PROXY_DNSBL'), ),
