...
class OptunaOptimizer:
    ...
    '''
    def __init__(self, model: Any, data: Tuple[pd.DataFrame, pd.Series], n_fold: int = 3,
                 early_stopping_rounds: int = 30, is_stratified: bool = True, is_shuffle: bool = True):
        self.model = model
        self.data = data
        self.n_fold = n_fold
        ...
        if self.is_stratified:
            self.cross_validation_split = StratifiedKFold(n_splits=self.n_fold, shuffle=self.is_shuffle)
        else:
            self.cross_validation_split = KFold(n_splits=self.n_fold, shuffle=self.is_shuffle)
    ...
    def _construct_trial_grid(trial: optuna.Trial, param_space: Dict):
        ...
        for name, params in param_space.items():
            ...
            if param_type == 'categorical':
                ...
                if len(params) > 4:
                    log = params[4]
                param_grid[name] = trial.suggest_float(name, low, high, step=step, log=log)
            elif param_type == 'int':
                low, high = params[1], params[2]
                step, log = 1, False
                if len(params) > 3:
                ...
                if len(params) > 4:
                    log = params[4]
                param_grid[name] = trial.suggest_int(name, low, high, step=step, log=log)
            else:
                raise ValueError(f'Undefined sampling method given for trial object: {name}: {params}')
    ...
    def _objective(self, trial: optuna.Trial, param_space: Dict, scoring: str):
        ...
        scores = cross_val_score(
            self.model, self.data[0], self.data[1], cv=self.cross_validation_split, scoring=scoring
        )
        return scores.mean()

    def optimize(self, param_space: Dict,
