...
class CatBoostModel(catboost.CatBoostClassifier):
    ...
    '''
    def __init__(self, categorical_columns: Any = None, text_columns: Any = None, **catboost_param):
        super().__init__(**catboost_param)
        self.categorical_columns = categorical_columns or catboost_param.get('cat_features')
        self.text_columns = text_columns or catboost_param.get('text_features')

    def fit(self, X: Union[pd.DataFrame, pd.Series],
            y: Any = None, **fit_params: Dict):
        train_pool = catboost.Pool(X, y, cat_features=self.categorical_columns,
    ...
    def predict(self, X: Union[pd.DataFrame, pd.Series], **predict_params: Dict):
        pool = catboost.Pool(X, cat_features=self.categorical_columns,
                             text_features=self.text_columns)
        return super().predict(pool, **predict_params)

    def predict_proba(self, X: Union[pd.DataFrame, pd.Series], **predict_proba_params: Dict):
        pool = catboost.Pool(X, cat_features=self.categorical_columns,
                             text_features=self.text_columns)
        return super().predict_proba(pool, **predict_proba_params)

    def score(self, X: Union[pd.DataFrame, pd.Series],
              y: Any = None):
    ...
    def evaluate(self, X_test: Union[pd.DataFrame, pd.Series], y_test: Any, metric: str, **eval_params: Dict):
        pool = catboost.Pool(X_test, y_test,
                             cat_features=self.categorical_columns,
                             text_features=self.text_columns)
        return super().eval_metrics(pool, [metric], **eval_params)

    def cv(self, X: Union[pd.DataFrame, pd.Series], y: Any,
           verbose: bool = False, **cv_params: Dict) -> Union[pd.DataFrame, Dict]:
        pool = catboost.Pool(X, y,
                             cat_features=self.categorical_columns,
                             text_features=self.text_columns)
