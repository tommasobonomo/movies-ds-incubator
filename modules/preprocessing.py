...
def create_target_feature(df: pd.DataFrame):
    ...
    df["movie_classification"] = df.apply(lambda row: target_classification(row["revenue"], row["budget"]), axis=1)


def print_stats(df, budget_col='budget', revenue_col='revenue'):
    """ Print several statistics and nan values of given dataset"""
    len_df = len(df)
    print(f'Lenght: {len_df} Available budget: {len_df - len(df[df[budget_col] == 0])}'
...
def list_column_to_long_format(dataframe: pd.DataFrame, column: str, delimiter: str = ",") -> pd.DataFrame:
